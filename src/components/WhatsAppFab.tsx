...
export function WhatsAppFab() {
  ...
  return (
    <a
      ...
      aria-label="Orçamento rápido no WhatsApp"
      className="group fixed bottom-5 right-5 z-50 flex items-center gap-2 rounded-full bg-[#25D366] py-3 pl-3 pr-4 text-white shadow-lg transition-transform hover:scale-105 hover:shadow-xl"
    >
      <span className="pointer-events-none absolute inset-0 -z-10 rounded-full border-2 border-[#25D366] opacity-75 animate-ping [animation-duration:1.5s]" />
      <span className="pointer-events-none absolute inset-0 -z-10 rounded-full border-2 border-[#25D366] opacity-50 animate-ping [animation-duration:1.5s] [animation-delay:0.5s]" />
      ...
      <span className="text-sm font-semibold leading-none">Orçamento rápido</span>
    </a>
  );
}
